...
hammerheads <- "model {
  ...
  for (i in 1:nsharks) {
    ...
    # Encounter rates: Teleosts by location
    teleost_G[i] ~ dnorm(mu_teleost_G, tau_teleost_G) T(0, )
    t_encounter_G[i] ~ dpois(teleost_G[i])
    
    teleost_A[i] ~ dnorm(mu_teleost_A, tau_teleost_A) T(0, )
    ...
    # Foraging success rates: Black-tips
  ...
  # Calculating mean across all individuals by location
  ...
  # Weighted priors by location
  mu_teleost_G <- (mu_teleost_sonar_G + mu_teleost_video_G) / 2
  tau_teleost_G <- tau_teleost_sonar_G + tau_teleost_video_G

  mu_teleost_sonar_G ~ dgamma(1.14795918, 0.07653061)
  tau_teleost_sonar_G ~ dgamma(2.777778, 555.555556)

  mu_teleost_video_G ~ dgamma(0.390625, 0.078125)
  tau_teleost_video_G ~ dgamma(0.01444619, 0.92455621)
  
  mu_teleost_A ~ dgamma(1.7777778, 0.2222222)
  tau_teleost_A ~ dgamma(0.7303675, 6.5739645)
...
write.csv(sumr, "JAGS_scripts/Model_output_GA.csv")
...
# encounter rates of teleosts from video (Atlantic)
plot(results, vars=c("mean_t_encounter_atlantic"), c("hist", "trace"))
...
plot(results, vars=c("mean_foraging_success_atlantic_50"), c("hist", "trace"))
plot(results, vars=c("mean_foraging_success_atlantic_40"), c("hist", "trace"))
plot(results, vars=c("mean_foraging_success_atlantic_30"), c("hist", "trace"))
